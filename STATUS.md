...
## Branch state
`main` current with origin at the **#39** merge. `feat/feed-floor-consolidation` merged + pruned (local branch + stale tracking ref; GitHub auto-deletes the remote head on merge, so only the local refs needed dropping). Also landed since #36: the status-doc refresh (#37) and `docs/redline-review-note` (#38). Earlier session branches (#34/#35 and prior) already merged + pruned.
...
3. Bill feed — built. `/bills/[id]` is **floor + ceiling complete** (PR #34); the **feed list (`/bills`) is now floor-consolidated onto tokens** (PR #39 — neutral pills, lucide, type scale). ⚠ Relevance is undercut by the `issue_tags` coverage gap (below).
