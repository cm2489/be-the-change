...
| 2026-05-01 | **Urgency-score / display-status mismatch: accept divergence for v1.1** | Urgency weights are uncalibrated anyway per the 2026-04-28 entry above; polishing one component prematurely is noise. Bills in their first 7 days display as `'introduced'` but rank against the `'committee'` urgency tier (~0.45) in the feed sort. Decision will be revisited as part of the full urgency calibration pass, not as a one-off. See `docs/deferred.md#urgency-score-display-status-mismatch`. | Recompute urgency at read-time alongside `deriveDisplayStatus` (rejected — calibration debt makes one-off polish premature). Add `'introduced'` urgency tier to `computeUrgencyScore` and write at sync time with a migration (rejected — same reason, plus migration cost). |
